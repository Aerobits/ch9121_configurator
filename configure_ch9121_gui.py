...
def ping_ip(ip_address):
    ...
    result = subprocess.run(["ping", param, "1", ip_address], capture_output=True, text=True, check=False)
    # print(result)
    ...
    while True:
        ...
        if event == "Sprawdź":
            ...
            if ip and validate_ip_input(ip):
                is_free = ping_ip(ip)
                message = f"Adres IP {ip} jest wolny" if is_free else f"Adres IP {ip} jest zajęty"
